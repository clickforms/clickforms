...
export function AcceptTeamInviteForm({ token }: { token: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      const res = await fetch(`/api/admin/team/accept/${encodeURIComponent(token)}`);
      ...
      setInvite(data as TeamInviteDetails);
  ...
  return (
    <form className="login-form" onSubmit={handleSubmit}>
      <div className="invite-summary">
        <p>
          You&apos;ve been invited to join <strong>Clickforms Admin</strong> as{' '}
      ...
      <label className="login-field" htmlFor="accept-team-invite-password">
        <span className="login-field-label">Choose a password</span>
        <PasswordField
          id="accept-team-invite-password"
