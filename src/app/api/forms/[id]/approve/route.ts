...
import { approveForm } from '@/lib/forms/form-workflow';
...
/** Marks a draft form as approved — required before publish (draft → approved). */
export async function POST(_request: Request, { params }: RouteContext): Promise<NextResponse> {
  try {
    ...
    const result = await withOrgContext(session.user.organizationId, async (tx) => {
      ...
      const { form: updatedForm, version } = await approveForm(tx, form);
