...
import { revertFormToDraft } from '@/lib/forms/form-workflow';
...
/** Returns an approved or published form to draft for editing. */
export async function POST(_request: Request, { params }: RouteContext): Promise<NextResponse> {
  try {
    ...
    const result = await withOrgContext(session.user.organizationId, async (tx) => {
      ...
      const { form: updatedForm, version } = await revertFormToDraft(tx, form);

      await logAudit(
        {
          organizationId: session.user.organizationId,
          actorUserId: session.user.id,
          action: 'form.revert_to_draft',
          entityType: 'form',
          entityId: form.id,
