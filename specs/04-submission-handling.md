...
- **Email notification**: on `status = submitted`, send an email (via Resend — superseded 2026-09-15, was originally planned as SES; see ARCHITECTURE.md §3) to the form's configured notification recipient(s) with a summary + link to view the submission in the admin dashboard. As built: also attaches a PDF copy (src/lib/forms/submission-notification.ts).
